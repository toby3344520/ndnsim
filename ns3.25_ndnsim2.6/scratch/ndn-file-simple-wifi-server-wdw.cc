/*
 * ndn-file-simple-wifi-server-wdw.cc
 *
...
//		  "Speed", StringValue ("ns3::UniformRandomVariable[Min=10|Max=20]"), //节点移动速度： 1m/s ~ 5.0m/s
...
//  mobility.EnableAscii (stream, nodes);
  ...
  // Consumer
  ndn::AppHelper consumerHelper("ns3::ndn::FileConsumerWdwTian");
...
//                                 MakeCallback(&FileDownloadStartedTrace));
  ...
  ndn::AppDelayTracer::InstallAll(string("app-delays-trace-wdw%.txt")+std::to_string(wifiRadius));
...
//  ss_tmp1 << "/home/tlx/simulation/FileConsumerLogTracer" << ".txt";
