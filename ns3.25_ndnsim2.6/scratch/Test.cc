...
static const uint32_t nwifis = 6;
double sum = 0;
double cnt = 0;
...
void installMobility(NodeContainer &c, int simulationEnd)
{
  ...
  if(test){
    ...
    for (uint32_t i = 2; i < 4; i++) {
          wayMobility[i] = c.Get(i)->GetObject<WaypointMobilityModel>();
          Waypoint waypointStart(Seconds(0), Vector3D((i-2)*25, 26, 0));
         // Waypoint waypointEnd(Seconds(simulationEnd), Vector3D(i*25, 0, 0));
    ...
    for (uint32_t i = 4; i < 5; i++) {
             wayMobility[i] = c.Get(i)->GetObject<WaypointMobilityModel>();
             Waypoint waypointStart(Seconds(0), Vector3D(-21, 13, 0));
            // Waypoint waypointEnd(Seconds(simulationEnd), Vector3D(i*25, 0, 0));
    ...
    for (uint32_t i = 5; i < 6; i++) {
               wayMobility[i] = c.Get(i)->GetObject<WaypointMobilityModel>();
               Waypoint waypointStart(Seconds(0), Vector3D(46, 13, 0));
              // Waypoint waypointEnd(Seconds(simulationEnd), Vector3D(i*25, 0, 0));
...
void installNDN(NodeContainer &c,int Size)
{
  ndn::StackHelper ndnHelper;
  ndnHelper.SetDefaultRoutes(true);
  ndnHelper.setCsSize(Size);
...
//	  consumerHelper.SetAttribute("WriteOutfile",StringValue("/home/sun-linux/Desktop/12323.mp3"));
	ns3::ndn::AppHelper consumerHelper("ns3::ndn::ConsumerCbr"); // ConsumerZipfMandelbrot可以产生重复的InterestPacket.
	  consumerHelper.SetPrefix("/myPrefix");
	  consumerHelper.SetAttribute("Frequency", ns3::DoubleValue(10));
	  //consumerHelper.SetAttribute("Batches", StringValue("1s 8 2s 8 3s 8 4s 8 5s 8 6s 1"));
...
// 	cmd.AddValue("Cache_Number", "the mount of cache", Cache_Number);
  ...
  int simulationEnd = 16;

  NodeContainer c;
  c.Create(nwifis);

  NodeContainer consumers;
    consumers.Add(c.Get(4));//s yb2018
...
////////////////////////////////////////////////////////////////////////////////
  //ndn::CsTracer::InstallAll("srcatch/cs-tracer.txt",Seconds(19.99));
  ns3::ndn::AppDelayTracer::Install(consumers,"scratch/app-Delay-trace.txt");
  ns3::ndn::L3RateTracer::InstallAll("scratch/rate.txt",Seconds(15));
  ///////////////////////
  Simulator::Run ();
/////////////////////////////////////////// statistic output
